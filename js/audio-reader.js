...
(function () {
  ...
  function init() {
    ...
    function start() {
      // Cancel previous utterance and wait for it to fully stop
      // (browsers fire onerror=interrupted otherwise)
      ...
      var startSpeak = function () {
        ...
        var v = pickVoice(); if (v) utterance.voice = v;
        ...
        utterance.onend = function () {
          if (state === 'idle') return; // already stopped manually
          state = 'idle'; showPlay();
        ...
        utterance.onerror = function (e) {
          ...
          console.warn('[audio-reader] utterance error:', code);
          // Ignore expected interruptions (cancel, manual stop, restart)
          if (code === 'canceled' || code === 'interrupted') return;
          state = 'idle'; showPlay();
          titleEl.textContent = 'Erreur — réessayez (' + code + ')';
          widget.classList.remove('playing');
        };

        // Workaround Chrome bug : speech stops after ~15s. Keep alive.
      ...
      // Wait 100ms after cancel before starting (Chrome needs time to clear state)
      if (wasIdle) {
        startSpeak();
      } else {
        setTimeout(startSpeak, 120);
      }
